...
def test_update_version_file() -> None:
    content = '__version__ = "0.0.24"\n'

    new_content = update_version_file(
        content, "0.0.25", Path("src/fastapi_cli/__init__.py")
    )

    assert new_content == '__version__ = "0.0.25"\n'
...
def test_update_version_file_requires_newer_version() -> None:
    ...
    with pytest.raises(RuntimeError, match="must be greater"):
        update_version_file(content, "0.0.24", Path("src/fastapi_cli/__init__.py"))
...
"""
    ...
    new_content = update_release_notes(
        content, "0.0.25", date(2026, 5, 30), Path("release-notes.md")
    )
...
"""
    ...
    body = get_release_notes_body(content, "0.0.25", Path("release-notes.md"))
...
def test_get_release_notes_body_requires_version_section() -> None:
    ...
    with pytest.raises(RuntimeError, match="Could not find"):
        get_release_notes_body(content, "0.0.25", Path("release-notes.md"))
...
def test_cli_updates_configured_files(tmp_path: Path) -> None:
    version_file = tmp_path / "src" / "fastapi_cli" / "__init__.py"
    version_file.parent.mkdir(parents=True)
    version_file.write_text('__version__ = "0.0.24"\n')
...
"""
    ...
    assert "Prepared release 0.0.25 (2026-05-30)" in result.output
    assert version_file.read_text() == '__version__ = "0.0.25"\n'
    assert "## 0.0.25 (2026-05-30)" in release_notes_file.read_text()
...
def test_cli_prints_current_version(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0, result.output
    assert result.output == "0.0.24\n"


def test_cli_prints_release_notes(tmp_path: Path) -> None:
    version_file = tmp_path / "src" / "fastapi_cli" / "__init__.py"
    version_file.parent.mkdir(parents=True)
    version_file.write_text('__version__ = "0.0.25"\n')
    release_notes_file = tmp_path / "release-notes.md"
    release_notes_file.write_text(
        """# Release Notes
